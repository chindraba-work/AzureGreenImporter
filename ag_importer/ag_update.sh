...
function fix_charset { # {{{
    ...
    src_file="$1"
    perl -CO -pe 'BEGIN{binmode STDIN,":encoding(cp1252)"}' <"$dir_new/$src_file.csv" >"$dir_import/utf8-${src_file,,}.csv"
    # Attempt to "clean" the data auto generated by Excel
    perl -pi -e 's/(\x0D|\x0A)+/\n/g;s/(^|\n)[\n\s]*/$1/g;s/,"""/,"”/g;s/""",/”",/g;s/"""\n/”"\n/g;s/([^,])""([^,\n])/$1”$2/g;s/\n([^"])/ $1/g;s/[^\S\n]{1,}([^\S]|")/$1/g' $dir_import/utf8-${src_file,,}.csv;
    if [[ 'Descriptions' == $src_file ]]; then
        perl -pi -e 's/([^"])\n/$1 /g' $dir_import/utf8-${src_file,,}.csv;
    fi
} # }}}
