...
function convert_data_file {
    ...
    src_file="$1"
    printf "Processing updated file '%s'.\n" $src_file
...
function freshen_annual_sheets {
    ...
    for annual_file in $(seq $YEAR_FIRST $YEAR_LAST); do
        printf "Checking master file '%s'.\n" "$annual_file master updates"
...
function freshen_change_sheets {
    ...
    for change_file in $CHANGE_LIST; do
      printf "Checking history file '%s'.\n" "$change_file"
...
function freshen_images {
    ...
    printf "Processing image archives.\n"
...
function get_images {
    ...
    printf "Searching for missing images.\n"
...
function retrieve_image {
    ...
    retrieve_new_image $target_file $target_path && { printf " ... Found\n"; return 0; }
...
function update_database {
    ...
    printf "Importing data into the database.\n"
...
[[ -n $2 ]] && printf "\n... Processing files for '%s' into directory '%s'.\n" $2 "$(realpath $1)"
main $1 $2
